...
theta_e = 10                                                # HC: added for dynamic synapse
u = 0.1                                                     # HC: added for dynamic synapse
alpha = 0.1                                                 # HC: added for dynamic synapse
...
##      (4) Y values
##      (5) Perplexity value
def lossFun(inputs, targets, hprev, eprev):
    xs, hs, ys, ps = {}, {}, {}, {}
    es = {}                                                             # HC: added for synamic synapse
    loss = 0
    counter = 0
    hs[-1] = np.copy(hprev)                                             # Assigning hprev to last column of hs for the upcoming loop
    es[-1] = np.copy(eprev)                                             # HC: added for synamic synapse
    ...
    for t in range(len(inputs)):
        ...
        efactor = np.multiply(hs[t-1], es[t-1])                         # HC: added for dynamic synapse
        hs[t] = np.tanh(np.dot(Wxh, xs[t]) + np.dot(Whh, efactor) + bh) # Hidden state calculation
        ...
        de = (-es[t-1] / theta_e) + u*hs[t-1]                           # HC: added for dynamic synapse
        es[t] = es[t-1] + alpha*de                                      # HC: added for dynamic synapse
    ...
    return loss, dWxh, dWhh, dWhy, dbh, dby, hs[len(inputs)-1], es[len(inputs)-1], ys, perplexity
...
def sampleFun(sampleData, hprev, eprev):
    inputs = sampleData[:-1]
    ...
    for t in range(len(inputs)):
        xs[t] = np.zeros((vocabSize,1))                                 # Encode input in a 1-of-k representation - set vector
        xs[t][inputs[t]] = 1                                            # Encode input in a 1-of-k representation - write '1' in correct index
        factor = np.multiply(hs[t-1], es[t-1])                          # HC: added for dynamic synapse
        hs[t] = np.tanh(np.dot(Wxh, xs[t]) + np.dot(Whh, factor) + bh)  # Hidden state calculation
        ys[t] = np.dot(Why, hs[t]) + by                                 # Un-normalized log probabilities for next char
        ps[t] = np.exp(ys[t]) / np.sum(np.exp(ys[t]))                   # Softmax function to determine probability vector for next char
        de = (-es[t-1] / theta_e) + u*hs[t-1]                           # HC: added for dynamic synapse
        es[t] = es[t-1] + alpha*de                                      # HC: added for dynamic synapse
...
while True:
    ...
    if  n == 0:
        ...
        eprev = np.ones_like(hprev)  # same size as h                           # HC: added for dynamic synapse - add efficiency factor for hidden state facilitation
        p = 0
    ...
    if p + seqLength + 1 >= len(data):                                                                      # HC - modified:  n % 100 == 0  --> p+seq_length+1 >= len(data)
        ...
        testIndices, testPerplexityIteration, testLossIteration = sampleFun(sampleData, hprev, eprev)       # HC
    ...
    loss, dWxh, dWhh, dWhy, dbh, dby, hprev, eprev, y, trainingPerplexityIteration = lossFun(inputs, targets, hprev, eprev)
